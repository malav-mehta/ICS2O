...
var Level = class Level {
  // levels are grid plans, which are then filled with vectors
  constructor(levelPlan) {
    // class constructor, which create the level plan in the form of characters comprehensible by the js file
    let levelInArray = levelPlan
      .trim()
      .split("\n")
    ...
    this.levelInArray = levelInArray.map((row, y) => {
      // mapping the level plan to create the actors on the level
      return row.map((character, x) => {
        // the two variables that will be used later for the creation of actors
        ...
        if (typeof type == "string") {
          return type;
        } // returns the type/class if the character is a string
...
var State = class State {
  // variable, in the form of a class, containing all information pertaining to the current status of the game
  constructor(level, actors, status) {
    // constructs the variable in a class constructor
  ...
  static start(level) {
    // static method for the class, used for creating objects throughout the file, in this case this creates a new State for the game, assigned to the state variable which will be passed in the main game loop
    return new State(level, level.gameActors, "playing"); // when the game begins, these values are used for the level
  }

  get player() {
    // get method for the class, used to retrieve information pertaining to the player
...
var Vector = class Vector {
  // the variable/class through which all actors are created in the form of vector based scalable graphics
  constructor(x, y) {
    // information containing the size of the actor
    this.x = x; // the x value of the vector actor
    this.y = y; // the y value of the vector actor
  }

  plus(other) {
    // plus function used to move vector object positions
  ...
  times(factor) {
    // times function used to scale the graphics with a predetermined factor
...
var Player = class Player {
  // player variable defined by the player class
  ...
  get type() {
    return "player";
  } // used in the creation of the level plan
...
var RedBlock = class RedBlock {
  // variables that is used to create lava actors, again the form of classes to minimize code redundancy
  constructor(position, speed, resetPosition) {
  ...
  get type() {
    return "RedBlock";
  } // used for the level plan creation, when creating actors
...
class Powerup {
  // powerups defined as separate classes (there are 5 different types of powerups)
  constructor(position, charactertype) {
    // constructor calls for and defines all values needed to create the powerups
  ...
  get type() {
    return "powerup";
  } // used for the level plan creation

  static create(position, character) {
    // static method for intial creation of powerups
  ...
  update() {
    // powerup is updated on the case of collision
    return new Powerup(this.position, this.charactertype); // the new powerup, generated based on the update
  }

  collide(state) {
    // the collision detector, which determines what to do on the case of collision
...
class Monster {
  // class for creating monsters (monsters are characters tha chase the player, and can be killed if the player jumps on top of them)
  ...
  get type() {
    // type used to create levels as well to fill in the color of the monster in the CSS code
    return "monster";
  }
  static create(position, character) {
    // determines whether or not the monster is to chase the player, as well as the starting position of the monster
    if (character == "m") {
  ...
  update(time, state) {
    // function that updates the monster every frame
    if (this.chase) {
      // if the monster is to chase the player, this code must be executed to determine the direction of the monster
      if (state.player.position.x < this.position.x) {
        // determines if player is on the left side of the monster
  ...
  collide(state) {
    // function that determines what happens when the player collides with the monster
    ...
    if (monster.position.y - player.position.y > 1) {
      // determines whether or not the player hit the monster on the top
...
var Coin = class Coin {
  // class used to create all the coins on the level (all coins must be collected to move on to the next level)
  constructor(position, intialposition, wobble) {
    // constructor creates the intial variables required for each coin
  ...
  get type() {
    return "coin";
  } // returns needed type for the level creation/css background fill
  ...
  static create(position) {
    ...
    ); // returns a new coin to the state gameActors list with the wobble accounted for
  }
};
...
var levelChars = {
  // every single character type is defined in this list, used by the var Level and the class Level to parse the intial level designs that are constructed in strings
...
function createElements(name, attributes, ...children) {
  // name = the element to create, list of attributes to set, children = list of children to append to the element shown on screen
  ...
  for (let attribute of Object.keys(attributes)) {
    // for loop that runs through all the attributes that are initially sent through the function (attributes are in the form of a dictionnary)
  ...
  for (let child of children) {
    // for loop that goes through all the children elements that are to be created on the screen (if applicable)
...
var gameScreen = class gameScreen {
  // the gameScreen variable holds the table, in which all the actors and the background is created
  constructor(documentElement, level) {
    this.shownOnScreen = createElements(
      "div",
      { class: "game" },
      drawGrid(level)
    ); // createElements called, new div generated, with the children being the grid drawn from the level
  ...
  clear() {
    this.shownOnScreen.remove();
  } // when gameScreen.clear() is called, the div is removed from the document, temporarily, so that the game can be updated
};

var scale = 25; // the scale of the game

function drawGrid(level) {
  // draws the table grid in which the gridded background of the game is created
  return createElements(
    "table",
    { class: "background", style: `width: ${level.width * scale}px` },
    ...level.levelInArray.map((row) =>
      createElements(
        "tr",
        { style: `height: ${scale}px` },
        ...row.map((type) => createElements("td", { class: type }))
      )
    )
  ); // creates the background div for the game
}

function drawActors(actors) {
  // draw the upper layer, in which the actors are generated more frequently than the background
  return createElements(
    "div",
    {},
    ...actors.map((actor) => {
...
gameScreen.prototype.syncState = function (state) {
  // the function, attached to the prototype of the game screen class, used to sync the state of the actors and update them
...
gameScreen.prototype.scrollPlayerIntoView = function (state) {
  // the function that handles the camera position by scrolling the player into view
...
Level.prototype.touches = function (position, size, type) {
  // collision detector for any element of the level
  ...
  for (var y = yStart; y < yEnd; y++) {
    // goes through all the y values of the element
    for (var x = xStart; x < xEnd; x++) {
      // for each y value, goes through all the x values of the element
...
State.prototype.update = function (time, keys) {
  // updates the State regularly based on the pressed keys and the time
  ...
  if (this.level.touches(player.position, player.size, "RedBlock")) {
    // check if player is touching a redblock
  ...
  for (let actor of actors) {
    // going through each actors
    if (actor != player && overlap(actor, player)) {
      // check if two actors are colliding//overlapping (different function that the player and an element from the background div)
...
function overlap(actor1, actor2) {
  // used to determine if two actors are overlapping (used for coin collection, killing monsters, killing player)
  return (
...
}; // when the player collides with the red block, the game is lost
...
Coin.prototype.collide = function (state) {
  ...
  if (!filtered.some((actor) => actor.type == "coin")) {
    // checks if there are any coins left in the actor list
    gameWon.play();
...
RedBlock.prototype.update = function (time, state) {
  // the redblocks are updated based on there movement type (as determined in the intitial redblock class)
...
Coin.prototype.update = function (time) {
  // updating the coin so that is wobbles
  ...
  return new Coin(
    this.intialposition.plus(new Vector(0, wobbleposition)),
    this.intialposition,
    wobble
  ); // the new, updated coin
};
...
Player.prototype.update = function (time, state, keys) {
  ...
  } // if the left arrow key is pressed, then the player speed is subtracted from the x speed so that the player moves to the left
  ...
  if (!state.level.touches(movedY, this.size, "wall")) {
    // if the player touches a redblock
    position = movedY; // don't let the player move
  } else if (
    (keys.w && ySpeed > 0) ||
    (keys.ArrowUp && ySpeed > 0) ||
    (keys.Space && ySpeed > 0)
  ) {
    // check if player jumped (but disable double jumps)
...
function trackKeys(keys) {
  // function which sets up the keys that need to be tracked for the game input
  let down = Object.create(null); // check is a key is down
  function track(event) {
    // tracks all events
    if (keys.includes(event.key)) {
      // if the keys variable includes the event key
...
function runAnimation(frameFunc) {
  ...
  function frame(time) {
    // the frame is generated as a function of time
    if (lastTime != null) {
      // if last time is not defined
...
function runLevel(level, Display) {
  // function which is used to run each individual level
  ...
  return new Promise((resolve) => {
    // new promis (required for the upcoming async function)
    runAnimation((time) => {
      ...
      if (state.status == "playing") {
        ...
        // otherwise unload the level, and return false
        display.clear();
...
async function runGame(plans, Display) {
  // async funciton used to run game (processes can occur in any order)
  for (let level = globalLevel; level < plans.length; ) {
    // for loop running through all the levels of the game
    let status = await runLevel(
      new Level(plans[level]), // awaiting the outcome of the level (to see whether player won or lost the level)
      Display
    );
    if (status == "won") {
    ...
    document.getElementById(
      "stats"
    ).innerHTML = `deaths: ${deaths}, level: ${globalLevel}, ratio: ${
      Math.round((globalLevel / deaths) * 100) / 100
    } (current session)`; // update users current state bar
